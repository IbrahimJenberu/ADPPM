...
async def broadcast_opd_assignment(
    doctor_id: uuid.UUID,
    ...
    """
    Broadcast an OPD assignment to all connected doctors via WebSocket.
    Also send via HTTP webhook as a fallback.
    """
    ...
    # Create message with complete patient data
    message = {
        "type": "patient_assignment",
        "doctor_id": doctor_id_str,
        "patient_id": patient_id_str,
        "assignment_id": assignment_id_str,
        "data": assignment_data,  # Include all assignment data
        "timestamp": datetime.now().isoformat()
    }
    
    # First try WebSocket broadcast
    delivered = False
    try:
        # Send to all WebSocket connections for this doctor
        delivered = await connection_manager.send_personal_message(doctor_id_str, message)
    except Exception as e:
        logger.error(f"WebSocket broadcast failed: {str(e)}")
    
    # If WebSocket failed or doctor is offline, use HTTP webhook as fallback
    if not delivered:
        try:
            # Send via webhook - include complete message
            async with httpx.AsyncClient() as client:
                webhook_url = f"{settings.DOCTOR_SERVICE_URL}/webhooks/opd-assignments"
                ...
                if response.status_code != 200:
                    ...
                    logger.info(f"Assignment delivered via webhook to doctor {doctor_id_str}")
                    delivered = True
        except Exception as e:
